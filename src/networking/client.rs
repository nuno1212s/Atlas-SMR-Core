...
pub trait SMRClientNetworkNode<NI, RM, D> where RM: Serializable, D: ApplicationData + 'static {
    ...
    /// Bootstrap the node
    async fn bootstrap(node_id: NodeId, network_info: Arc<NI>, config: Self::Config) -> Result<Self> where Self: Sized;
}
...
impl<CN, NC, NI, RM, D> SMRClientNetworkNode<NI, RM, D> for CLINodeWrapper<CN, NC, NI, RM, D>
    ...
    async fn bootstrap(node_id: NodeId, network_info: Arc<NI>, config: Self::Config) -> Result<Self> {
        todo!()
    }
}